...
if (!function_exists('autoLoader')) {
    ...
    function requireFile($classFile, $class){
        $ret = false;
        ...
        if (!$clasWasFound) {
            ...
            foreach ($scanCurrentDirList as $dir) {
                if (is_dir($dir)) {
                    ...
                    // si no está, listamos los directorios nietos y buscamos entre ellos
                    $scanChildDirList = array_slice(scandir($dir), 2);
                    foreach ($scanChildDirList as $chilDir) {
                        if(is_dir($dir . "/" . $chilDir)){
                            $fullPath = $dir . "/" . $chilDir. "/" . $classFile;
                            if (is_file($fullPath) && !class_exists($class)) {
                                require_once($fullPath);
                                $clasWasFound = true;
                                break;
