...
$varLegal_legal = "0";
if (isset($_GET["idLegal"])) {
  $varLegal_legal = $_GET["idLegal"];
}
mysql_select_db($database_conexionturismo, $conexionturismo);
$query_legal = sprintf("SELECT tbllegal.strNombre, tbllegal.strSEO, tbllegal.strTexto FROM tbllegal WHERE tbllegal.intEstado = 1 AND tbllegal.strSEO = %s", GetSQLValueString($varLegal_legal, "text"));
$legal = mysql_query($query_legal, $conexionturismo) or die(mysql_error());
$row_legal = mysql_fetch_assoc($legal);
$totalRows_legal = mysql_num_rows($legal);
?>
<!DOCTYPE html>
<html lang="es-ES">
    <head>
        <?php include("includes/metatag.php"); ?>
        <meta property="og:title" content="<?php echo $row_legal['strNombre']; ?> de Caribetour.es | Especialistas en el Caribe" />
        <meta name="title" content="<?php echo $row_legal['strNombre']; ?> de CaribeTour.es: Especialistas en el Caribe" />
        <meta name="DC.title" content="<?php echo $row_legal['strNombre']; ?> de CaribeTour.es: Especialistas en el Caribe" />
        <title><?php echo $row_legal['strNombre']; ?> de CaribeTour.es | Especialistas en el Caribe</title>        
        <meta name="description" content="<?php echo $row_legal['strNombre']; ?> de CaribeTour.es: Especialistas en el Caribe" />
        <meta name="keywords" content="<?php echo $row_legal['strNombre']; ?> de CaribeTour.es: Especialistas en el Caribe" />
        <!--[if lt IE 9]>
          ...
          <section class="container site-content">
          	<!--inicio breadcrumb-->
                <div class="miga" >
                    ...
                    <div class="breadcrumb">	
                        <a href="legal" rel="tag" title="Legal">Legal</a>
                    </div>
                    <div class="breadcrumb"><?php echo $row_legal['strNombre']; ?></div>
                </div>
            <!--fin de breadcrumb-->
            <div class="row">
                <div class="column ninecol">
                        <div class="items-list clearfix">
                          <h1><?php echo $row_legal['strNombre']; ?></h1>
                          <?php echo $row_legal['strTexto']; ?>
                        </div>
                      </div>
